              ...
              <div class="col-md-12 grid-margin">
                <div class="row">
                  <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                    <h3 class="font-weight-bold">Membership Plan</h3>
                       ...
                       if(mysqli_num_rows($query_run) > 0)
                       {
                           foreach($query_run as $membership)
                           {
                               ...
                               <tr>
                                   ...
                                   <td><?= $membership['signup_fees']; ?></td>
                                   <td><?= $membership['period']; ?></td>
                                   <td>
                                       <a href="membershipplan-edit.php?id=<?= $membership['membership_id']; ?>" class="btn btn-dark btn-sm">Edit</a>
                                       <form action="code.php" method="POST" class="d-inline">
                                     <button type="button" class="btn btn-danger btn-sm"  data-bs-toggle="modal" data-bs-target="#staticBackdrop<?=$membership['membership_id'];?>">
                                                                     Delete
                                                                   </button>
                                           <!-- Modal -->
                                     <div class="modal fade" id="staticBackdrop<?=$membership['membership_id'];?>" data-bs-backdrop="static" value="<?=$membership['membership_id'];?>" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                       <div class="modal-dialog">
                                         <div class="modal-content">
                                           ...
                                           <div class="modal-body">
                                             <p>Deleting <?= ucfirst($membership['Name']); ?> Package, are you sure?</p>
                                           </div>
                                           <div class="modal-footer">
                                             <button type="button" class="btn btn-dark btn-sm" data-bs-dismiss="modal">Close</button>
                                             <button type="submit" name="delete_membership" value="<?=$membership['membership_id'];?>" class="btn btn-danger btn-sm">Delete</button>
                                           </div>
                                         </div>
                                       </div>
                                     </div>
